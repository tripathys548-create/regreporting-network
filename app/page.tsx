...
export default async function HomePage() {
  ...
  // Stats and community modules only appear once they have something to show.
  const updateStat = updatesThisWeek > 0 ? { value: updatesThisWeek, label: "Regulatory updates this week" } : { value: updates.length, label: "Published regulatory updates" };
  const communityStat = activeDiscussions > 0 ? { icon: "message" as const, value: activeDiscussions, label: "Active discussions (7 days)", href: "/community" } : { icon: "shield" as const, value: radarSources.length, label: "Official sources monitored", href: "/radar" };
        ...
        <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1.35fr_1fr] lg:gap-12">
          <div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-3">
              <ButtonLink href="/radar" variant="primary" size="md" iconRight="arrowRight">
                Explore Regulatory Updates
              </ButtonLink>
              <Link href="/community/new" className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-200 underline-offset-4 hover:text-white hover:underline">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <HeroStat icon="radar" value={String(updateStat.value)} label={updateStat.label} href="/radar" />
              <HeroStat icon={communityStat.icon} value={String(communityStat.value)} label={communityStat.label} href={communityStat.href} />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="min-w-0 space-y-6 lg:col-span-2">
          {trending.length > 0 ? (
            ...
            <section className="flex flex-col gap-4 rounded-md border border-line bg-surface p-5 sm:flex-row sm:items-center sm:justify-between" aria-labelledby="community-start-heading">
              <div className="flex items-start gap-3">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-accent-soft text-accent">
                  <Icon name="message" />
                </span>
                <div>
                  <h2 id="community-start-heading" className="text-sm font-semibold text-ink">
                    Start the first practitioner discussion
                  </h2>
                  <p className="mt-0.5 text-xs text-body">Ask how other firms handle a validation reject, a UTI break or a new reporting change.</p>
                </div>
              </div>
              <ButtonLink href="/community/new" size="sm" variant="primary" icon="plus" className="shrink-0">
                New discussion
              </ButtonLink>
            </section>
          )}
            ...
            <ul className="grid gap-1.5 sm:grid-cols-2">
              {articles.slice(0, 8).map((a) => (
                <li key={a.id} className="min-w-0">
                  <Link href={`/knowledge/${a.slug}`} className="flex items-center justify-between gap-3 rounded border border-line px-2.5 py-2 text-xs font-medium text-ink hover:border-accent/40 hover:text-accent">
          ...
          <div className={mostDiscussed.length > 0 ? "grid grid-cols-1 gap-6 md:grid-cols-2" : undefined}>
