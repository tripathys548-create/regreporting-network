...
export type AdminSection = "updates" | "sources" | "reports" | "moderation" | "users" | "audit" | "knowledge" | "challenges";
...
export async function getAdminOverview(): Promise<AdminSectionSummary[]> {
  const [updates, pendingUpdates, feeds, failingFeeds, reports, openReports, removed, users, unverified, audit] = await Promise.all([
